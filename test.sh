...
assert "status has SPECING suggestion" grep -q 'SPECING' "$STATUS_SKILL"
assert "status SPECING mentions milestone" grep -q 'gig:milestone' "$STATUS_SKILL"
assert "status reads SPEC.md" grep -q 'SPEC.md' "$STATUS_SKILL"
assert "status shows spec progress stories" grep -q 'stories' "$STATUS_SKILL"
assert "status shows spec progress requirements" grep -q 'requirements' "$STATUS_SKILL"
assert "status shows DRAFT status" grep -q 'DRAFT' "$STATUS_SKILL"
assert "status shows LOCKED status" grep -q 'LOCKED' "$STATUS_SKILL"
assert "status shows Coverage" grep -q 'Coverage' "$STATUS_SKILL"
assert "milestone has resume guard for SPECING" grep -q 'Resume elicitation' "$SCRIPT_DIR/skills/milestone/SKILL.md"
